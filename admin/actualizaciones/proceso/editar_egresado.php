...
?>
							...
							<div class="card-body">
							    ...
							    <div class="form-group">
								  ...
								  <div class="form-group">
									...
									<input type="text" name="email" class="form-control"  value="<?php echo $fila['email_persona']; ?>">
								  </div>
		...
		if (isset($_POST['persona_id'])) {
			...
			$actualizar_egresado = $pdo->prepare("UPDATE persona SET dni_persona=:dni, nombres_persona=:nombre, email_persona=:email, apellidos_persona=:apellido, carrera=:carrera, mencion=:mencion, modalidad=:modalidad, jornada=:jornada, pais=:pais WHERE  persona_id=:persona_id");

			$actualizar_egresado->bindParam(':dni', $dni_persona);
			$actualizar_egresado->bindParam(':nombre', $nombre);
			$actualizar_egresado->bindParam(':email', $email);
			...
			$actualizar_egresado->bindParam(':pais', $pais);
